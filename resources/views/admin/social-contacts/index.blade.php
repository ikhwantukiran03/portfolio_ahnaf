...
<div class="space-y-8">
    ...
    <div class="bg-card-white rounded-2xl p-8 shadow-sm border border-gray-100">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-3xl font-bold text-text-primary mb-2">Contacts & Links</h1>
    ...
    @if($contacts->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            @foreach($contacts as $typeKey => $typeContacts)
                <div class="bg-card-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <div class="flex items-center mb-6">
                        ...
                        @endphp
                        <div class="w-12 h-12 bg-primary-blue bg-opacity-10 rounded-xl flex items-center justify-center mr-4 flex-shrink-0">
                            <i class="{{ $displayIcon }} text-primary-blue text-xl"></i>
                        </div>
                        <div class="min-w-0 flex-1">
                            <h3 class="text-xl font-bold text-text-primary">{{ $displayType }}</h3>
                            <p class="text-text-secondary">{{ $typeContacts->count() }} contact(s)</p>
                        </div>
                    </div>

                    <div class="space-y-4" id="contacts-{{ Str::slug($typeKey) }}">
                        @foreach($typeContacts as $contact)
                            <div class="bg-gray-50 rounded-xl p-4 border border-gray-100 hover:border-primary-blue transition-all duration-200 contact-item" data-id="{{ $contact->id }}">
                                <div class="flex items-start justify-between gap-3">
                                    <div class="flex-1 min-w-0">
                                        <!-- Contact Header -->
                                        <div class="flex items-center flex-wrap gap-2 mb-3">
                                            <i class="{{ $contact->display_icon }} text-primary-blue flex-shrink-0"></i>
                                            <span class="font-semibold text-text-primary truncate">{{ $contact->label }}</span>
                                        ...
                                        <div class="mb-3">
                                            @if(in_array($contact->type, ['email', 'phone']))
                                                <a href="{{ $contact->url }}" class="text-primary-blue hover:text-light-blue font-medium break-all">
                                                    {{ $contact->display_value }}
                                                </a>
                                            @else
                                                <a href="{{ $contact->url }}" target="_blank" class="text-primary-blue hover:text-light-blue font-medium break-all">
                                                    {{ $contact->display_value }}
                                        ...
                                        @if($contact->description)
                                            <p class="text-sm text-text-secondary mb-3 line-clamp-2">{{ $contact->description }}</p>
                                        @endif

                                        <!-- Status -->
                                        <div class="flex items-center justify-between">
                                            <span class="px-3 py-1 text-xs font-medium rounded-full {{ $contact->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
    ...
    @else
        ...
        <div class="bg-card-white rounded-2xl p-12 shadow-sm border border-gray-100 text-center">
            <div class="w-24 h-24 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-6">
                <i class="fas fa-address-book text-3xl text-gray-400"></i>
            </div>
            <h3 class="text-2xl font-bold text-text-primary mb-4">No Social Contacts Yet</h3>
    ...
    @if($contacts->count() === 0)
        <div class="bg-card-white rounded-2xl p-8 shadow-sm border border-gray-100">
            <h2 class="text-2xl font-bold text-text-primary mb-6 text-center">Available Contact Types</h2>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                @foreach($contactTypes as $typeKey => $typeInfo)
                    <div class="bg-gray-50 rounded-xl p-4 text-center border border-gray-100 hover:border-primary-blue transition-colors">
                        <div class="w-12 h-12 bg-primary-blue bg-opacity-10 rounded-xl flex items-center justify-center mx-auto mb-3">
                            <i class="{{ $typeInfo['icon'] }} text-primary-blue"></i>
                        </div>
                        <p class="text-sm font-medium text-text-primary">{{ $typeInfo['label'] }}</p>
                    </div>
                @endforeach
            </div>
        </div>
